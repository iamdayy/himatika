...
const { data: users, refresh } = await useAsyncData(() => $fetch<IUser[]>("/api/user"));
onMounted(() => {
    initModals()
})
</script>
            ...
            <tbody>
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                    ...
                    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                        <NuxtImg class="rounded-full" :src="user?.profile.avatar || '/profile-blank.png'" sizes="40"
                            alt="Jese image" />
                        <div class="ps-3">
                            <div class="text-base font-semibold">{{ user.username }}</div>
                            <div class="font-normal text-gray-500">{{ user.profile.email }}</div>
                        </div>
                    </th>
                    <td class="px-6 py-4">
