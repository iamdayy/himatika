...
const departements = ref<IDepartement[] | undefined>(data.value?.filter((dep) => new Date(dep.period.start).getFullYear() >= getStartYear(option.value) && new Date(dep.period.end).getFullYear() <= getEndYear(option.value)));
