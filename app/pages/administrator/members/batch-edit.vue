...
import { useBatchMemberImport } from '~/composables/useBatchMemberImport';
...
const { RawData, availableColumns, columnMapping, autoDetectMapping, processMapping, cancelMapping: resetMappingData } = useBatchMemberImport();
...
const applyMapping = () => {
    try {
        DataFromCSV.value = processMapping();
        selectedRows.value = {
...
const addCollegers = async () => {
    ...
    try {
        ...
        if (added.data) {
            ...
            selectedRows.value = {}; // Reset selection to prevent out-of-sync UI
        ...
        <UCard class="p-2 mt-2 md:p-4">
            ...
            <div v-if="currentStep === 2" class="px-2 py-4 md:px-8">
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-500">Pilih kolom Excel yang sesuai. Untuk Batch Edit, hanya kolom NIM
                        yang wajib.
                        Kolom lain yang dikosongkan tidak akan mengubah data yang ada.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <UFormField label="Nama Lengkap">
                        <USelect v-model="columnMapping.fullName" :items="availableColumns"
                            placeholder="Biarkan kosong jika tak diubah..." class="w-full" />
                    </UFormField>
