...
<template>
    <div class="space-y-6 mb-24">
        ...
        <div v-else-if="requests?.length === 0" class="text-center text-gray-500 py-10 border rounded-lg border-dashed">
            Tidak ada pengajuan prestasi baru.
        </div>

        <UCard v-for="req in requests" :key="req._id">
            <div class="flex flex-col md:flex-row gap-4 items-start">
                <div class="w-full md:w-48 h-32 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0 border">
                    <img v-if="req.proof" :src="req.proof" class="w-full h-full object-cover" />
                    <div v-else class="flex items-center justify-center h-full text-gray-400 text-xs">No Image</div>
                </div>
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ req.member?.fullName }} • {{ req.member?.NIM }}</p>
                        </div>
                        <UBadge :label="req.type" color="secondary" variant="subtle" />
                    </div>

                    <p class="mt-2 text-sm bg-gray-50 p-2 rounded">{{ req.description }}</p>
                ...
                <div class="flex flex-col gap-2 min-w-[100px]">
                    <UButton color="success" icon="i-heroicons-check" block @click="openApprove(req)">Proses</UButton>
        ...
        <UModal v-model="approvalForm.isOpen">
            <UCard>
                <template #header>
                ...
                <div class="space-y-5 max-h-[70vh] overflow-y-auto px-1">
                    <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                        <UFormField label="Berikan Poin Reward" help="Nilai poin yang akan masuk ke akumulasi member">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <UToggle v-model="approvalForm.createNews" />
                    </div>
                    ...
                    <div v-if="approvalForm.createNews" class="space-y-4 border-l-2 border-primary pl-4 ml-1">
                        ...
                        <UFormField label="Isi Berita (HTML)">
                            <UTextarea v-model="approvalForm.newsBody" :rows="6" />
                            <p class="text-xs text-gray-400 mt-1">Tips: Gunakan tag &lt;p&gt;, &lt;strong&gt; untuk
                                format.</p>
                        </UFormField>
                    </div>
                </div>
