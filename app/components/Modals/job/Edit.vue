...
/**
 * Initialize new job object with default values
 * @type {IJob}
 */
const stateJob: IJob = reactiveComputed<IJob>(() => ({
    label: props.job.label || '',
    count: props.job.count || 1,
}));
...
<template>
    <UModal :fullscreen="windowSize.width.value < 640" label="Edit Job">
        ...
        <template #footer>
            <div class="flex items-center justify-between w-full">
                <UButton @click="emit('close')" label="Close" icon="i-heroicons-x-mark" variant="ghost" color="error"
                    :size="buttonSize" />
                <UButton type="submit" @click="emit('submit', stateJob)" label="Save" icon="i-heroicons-clipboard"
